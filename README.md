...
- `--prediction_mode`: What the network estimates — `residual` (default) or `direct`. In **residual** mode it predicts the per-voxel *correction* applied to the input (`output = input + unet(input)`); where the network predicts ~0 the output equals the input exactly, which bounds the small systematic mean drift that direct prediction can accumulate. In **direct** mode it reconstructs the denoised volume outright (`output = unet(input)`) — the original formulation, kept available for comparison. The two modes are different architectures with incompatible checkpoints, so switching **requires retraining**. Recorded in `params.json` and read back automatically at inference.
