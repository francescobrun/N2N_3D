...
- `--augment_symmetry`: Which cube symmetries to draw training augmentation from — `inplane` (default) or `full`.
  - **`inplane`** — the 8 rotations that keep z on axis 0, plus the 50% horizontal flip: **16 orientations**, none of which swap the gantry axis into the imaging plane. With isotropic voxels such a swap is geometrically valid, but CT reconstruction *noise* is not isotropic — streaks, rings and cupping are in-plane phenomena, while noise is far more independent across slices. Axis-swapped samples therefore teach the network to expect in-plane artifact structure along z, where it never occurs; actively misleading for a denoiser whose entire job is modelling noise structure.
  - **`full`** — all 24 rotations plus the flip, i.e. the full 48-element octahedral group. This was the behaviour before the flag existed. Triples augmentation diversity, and is the right choice only if your noise really is direction-agnostic.

  `inplane` additionally guarantees that the last two tensor dimensions are always the y-x imaging plane, which is what `--rotation_loss` depends on — under `full`, roughly two-thirds of samples arrive with the gantry axis rotated into that plane, so the rotation loss would be operating on the wrong plane most of the time. **Changing this changes training**, so runs are not comparable across settings.
- `--rotation_loss`: Add a rotation-based term to the training loss — `none` (default), `equivariance`, or `ran2i`. Both non-default variants are **experimental and unvalidated on this pipeline's data**. Both act only in the x-y plane (z held fixed) rather than full 3D, since z is this pipeline's physically distinguished gantry axis (see `training_circle_mask`'s cylindrical field-of-view geometry above) and only in-plane rotation is physically meaningful.
  - **`equivariance`** — CT reconstruction is approximately equivariant to rotating the scanned object: rotate the object and the reconstruction rotates identically, because the acquisition geometry has no preferred in-plane orientation. This forces the network to respect that property by penalizing the difference between `f(rotate(x))` and `rotate(f(x))` for 2 randomly chosen 90°/180°/270° rotations per step. Costs 2 extra forward passes (**~3x total training compute**).
  - **`ran2i`** — reproduces Xu & Perelli, ["Rotational Augmented Noise2Inverse"](https://arxiv.org/abs/2312.12644) (IEEE TRPMS 2023) as implemented in the authors' [reference code](https://github.com/UoD-MCI/RAN2I): rotate the network *output* and the *target* by 4 random continuous angles and compare. Requires **no extra forward pass**, so it is nearly free.

  The two are genuinely different mechanisms, and it is worth being clear about which. `ran2i` **cannot** constrain equivariance: the network is never evaluated on a rotated input, so nothing in the loss observes how `f` behaves under rotation. Because rotation is linear, its term reduces to `‖rotate(pred − target)‖²` — bit-identical to the primary loss at exact 90° angles, and at other angles a *frequency-reweighting* of it, since bilinear resampling attenuates high-frequency error roughly 2x more than low-frequency error while the corners rotate out of frame. That reweighting may well be useful for denoising (in Noise2Noise the high-frequency residual is dominated by the target's irreducible independent noise), but it is not the equivariance constraint the paper describes. It is provided because it is the formulation that produced the published results; `equivariance` is provided because it is the property the paper claims to enforce.
