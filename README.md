...
- `--rotation_equivariance`: Add a rotation-equivariance term to the training loss (default: **disabled**, experimental). Inspired by Xu & Perelli, ["Rotational Augmented Noise2Inverse"](https://arxiv.org/abs/2312.12644) (IEEE TRPMS 2023): CT reconstruction is approximately equivariant to rotating the scanned object, since the acquisition geometry has no preferred in-plane orientation. For 2 randomly chosen 90°/180°/270° rotations per training step, both the input and target patch are rotated together and the network is trained to hit the rotated target on the rotated input, added unweighted to the ordinary loss. Restricted to the x-y plane (z held fixed) rather than full 3D — z is this pipeline's physically distinguished gantry axis (see `training_circle_mask`'s cylindrical field-of-view geometry above), so only in-plane rotation is physically meaningful. Adds 2 extra forward passes per step (**~3x total training compute** when enabled). Needs empirical validation on real data before it's worth adopting as a default — treat it as an experiment, not a recommendation.
